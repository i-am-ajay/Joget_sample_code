...
public class EquipmentReturnCode {
    public static Object execute(WorkflowAssignment assignment, AppDefinition appDef, HttpServletRequest request) {
        ...
        // get do details
        LogUtil.info("Warranty Refrence",warrantyId);
        ...
        if(warranty != null ) {
            ...
            for (String doNum : doNumber.split(",")) {
                ...
                for(FormRow equip : rowsEqpMasterData){
                    ...
                    LogUtil.info("Equipment Number",eNum);
            ...
            /*doString.deleteCharAt(doString.length()-1);
            doNumber = doString.toString();
            LogUtil.info("DO Number",doNumber);
            FormRowSet rowsEqpMasterData = formDataDao.find("equipment_master", "equipment_master", "where c_do_Number =? ", new Object[] {doNumber}, null, null, null, null);
            for(FormRow equip : rowsEqpMasterData){
            ...
            equipmentNumber = equipmentNum.toString();
            LogUtil.info("Equipment Number",equipmentNumber);*/
        }
        LogUtil.info("DO List Size",doList.size()+"");
        LogUtil.info("Equipment List Size",equipmentNumberList.size()+"");
        if(doList.size() == 0){
            return null;
        }
        ...
        try {
            ...
            if(!con.isClosed()) {
                ...
                LogUtil.info("Query",retEqpstmt.toString());
