...
public class NoteIntegration {
    ...
    public static void dbCall() throws InterruptedException {
        ...
        FormRow oldRow = dao.load(formId, tableName, "N_"+id);
        ...
        try {
            ...
            // update items
            NoteIntegration.InvoiceItemIntegration.dbCall(id,headerId,recordUpdated,currencyCode);
        }
        catch(Exception ex){
    ...
    public static String headerTableQuery(){
        ...
        return "SELECT 'N_'||CSRH_SYS_ID as header_id, 'reporting:1.0' as process_control,CSRH_COMP_CODE || '/' || CSRH_TXN_CODE || '/' || CSRH_NO as invoice_identifier,  \n" +
                "                CSRH_UUID as UUID, CSRH_DT as issue_date,CSRH_TYPE_CODE type_code, \n" +
                "                CSRH_TXN_CODE_STRING as transaction_code,CSRH_ANNOTATION as notes,'' special_tax_treatment, \n" +
                "                '' purchase_order, CSRH_REF_TXN_CODE||'-'||CSRH_REF_NO as billing_ref_id, (SELECT DISTINCT CSRR_CR_DT FROM OT_CUST_SALE_RET_REF WHERE CSRR_CSRH_SYS_ID = CSRH_SYS_ID) billing_ref_date,  \n" +
                "                (SELECT DISTINCT INVH_LPO_NO FROM JOG_OT_INVOICE_HEAD WHERE INVH_SYS_ID = (SELECT DISTINCT CSRR_REF_SYS_ID FROM OT_CUST_SALE_RET_REF WHERE CSRR_CSRH_SYS_ID = CSRH_SYS_ID)) as contract_id, CSRH_NO invoice_counter,'' as seller_group_vat,  \n" +
                ...
                "                '' as buyer_additional_street, BuyerAddress.ADDR_FLEX_15 buyer_building_number, '' as buyer_additional_number, \n" +
                ...
                "                CSRH_CUST_NAME buyer_name, (SELECT DISTINCT INVH_DEL_DT FROM JOG_OT_INVOICE_HEAD WHERE INVH_SYS_ID = (SELECT DISTINCT CSRR_REF_SYS_ID FROM OT_CUST_SALE_RET_REF WHERE CSRR_CSRH_SYS_ID = CSRH_SYS_ID)) as supply_date, (SELECT INVH_DEL_DT FROM JOG_OT_INVOICE_HEAD WHERE INVH_SYS_ID IN (SELECT CSRR_REF_SYS_ID FROM OT_CUST_SALE_RET_REF WHERE CSRR_CSRH_SYS_ID = CSRH_SYS_ID)) as supply_end_date,  \n" +
                "                'Instrument not definedInstrument not defined' as payment_means_type_code, CSRH_FLEX_08 as dc_note_reason, '' as payment_terms, \n" +
                "                'false' allowance_indication, CSRH_DISC_PERC disc_per, CSRH_FC_DISC_VAL*CSRH_EXGE_RATE disc_val,  \n" +
                "                '0.0' as base_amount, 'S' as allowance_vat_code, (SELECT DISTINCT ITED_TED_RATE FROM JOG_OT_CUST_SALE_RET_ITEM_TED WHERE ITED_H_SYS_ID = JOG_OT_CUST_SALE_RET_HEAD.CSRH_SYS_ID AND ITED_TED_TYPE_NUM = 1) as allowance_vat_rate,  \n" +
                "                'VAT' as tax_scheme, '' advance_paid_amount, '' as vat_exemption_code, \n" +
                "                '' as vat_exemption_reason,CSRH_CURR_CODE currency_code, CSRH_COMP_CODE as company_code,  \n" +
                "                'TIN' as buyer_id_scheme, FM_COMPANY.COMP_FLEX_07 as other_seller_id_scheme, '' seller_additional_number\n" +
                "                 FROM \n" +
                "                JOG_OT_CUST_SALE_RET_HEAD \n" +
                "                LEFT JOIN JOG_OM_ADDRESS ON JOG_OT_CUST_SALE_RET_HEAD.CSRH_BILL_TO_ADDR_CODE = JOG_OM_ADDRESS.ADDR_CODE  \n" +
                "                LEFT JOIN JOG_OM_ADDRESS BuyerAddress ON JOG_OT_CUST_SALE_RET_HEAD.CSRH_COMP_CODE = BuyerAddress.ADDR_CODE  \n" +
                "                LEFT JOIN FM_COMPANY ON JOG_OT_CUST_SALE_RET_HEAD.CSRH_COMP_CODE = FM_COMPANY.COMP_CODE \n" +
                "                 WHERE  CSRH_SYS_ID = ?\n";
    }
    ...
    public static class InvoiceItemIntegration {
        ...
        public static FormRow populateRowFromRecord(ResultSet set,String currencyCode,boolean recordUpdated, String headerUuid) throws SQLException {
            ...
            LogUtil.info("Calling Populate Row","----------");
            FormRow row = new FormRow();
            ...
            row.setProperty("il_item_buyer_item_code",processColumn(set.getString("CSRI_ITEM_CODE")));
            row.setProperty("il_item_seller_item_code",processColumn(set.getString("CSRI_ITEM_CODE")));
            row.setDateCreated(new Date());
            return row;
        }
        ...
        public static String lineItemQuery(){
            return "SELECT 'N_'||CSRI_SYS_ID AS id, CSRI_SYS_ID, 'N_' || CSRI_CSRH_SYS_ID AS HEADER_ID,QTY, ITEM_UOM,Net_Amount,'false' discount_indicator,discount_perc,DISCOUNT,VAT_AMT,Amount_with_VAT,DESCRIPTION,CSRI_ITEM_CODE,RATE-0 item_net_price,  \n" +
                    ...
                    "                       FROM (SELECT\n" +
                    "                    (\n" +
                    "                           SELECT (CSRI_QTY_BU/IU_MAX_LOOSE_1)*IU_CONV_FACTOR FROM OM_ITEM, OM_ITEM_UOM \n" +
                    "                            WHERE ITEM_CODE  = IU_ITEM_CODE AND ITEM_UOM_CODE = IU_UOM_CODE AND CSRI_ITEM_CODE = ITEM_CODE\n" +
                    "                          ) QTY,  \n" +
                    "\n" +
                    "                    JOG_OT_CUST_SALE_RET_ITEM.CSRI_SYS_ID,JOG_OT_CUST_SALE_RET_ITEM.CSRI_CSRH_SYS_ID, JOG_OT_CUST_SALE_RET_ITEM.CSRI_ITEM_CODE, \n" +
                    "                    JOG_OT_CUST_SALE_RET_ITEM.CSRI_UOM_CODE AS ITEM_UOM, JOG_OT_CUST_SALE_RET_ITEM.CSRI_RATE RATE, \n" +
                    "                    JOG_OT_CUST_SALE_RET_ITEM.CSRI_ITEM_DESC AS DESCRIPTION,   \n" +
                    "                    TED.VAT_RATE AS VAT_PERC, \n" +
                    "                    TED.DISC AS DISCOUNT, CSRI_DISC_PERC discount_perc,\n" +
                    "                    TED.VAT_AMOUNT AS VAT_AMT,\n" +
                    "                    (CSRI_FC_VAL_AFT_H_DISC * CSRH_EXGE_RATE + TED.VAT_AMOUNT ) AS Amount_with_VAT,\n" +
                    "                    (CSRI_FC_VAL_AFT_H_DISC * CSRH_EXGE_RATE) AS Net_AMOUNT,\n" +
                    "                          JOG_OT_CUST_SALE_RET_ITEM.CSRI_QTY_BU\n" +
                    "\n" +
                    ...
                    "\n" +
                    "                    FROM JOG_OT_CUST_SALE_RET_ITEM\n" +
                    "                    JOIN JOG_OT_CUST_SALE_RET_HEAD ON JOG_OT_CUST_SALE_RET_ITEM.CSRI_CSRH_SYS_ID = JOG_OT_CUST_SALE_RET_HEAD.CSRH_SYS_ID\n" +
                    "                    JOIN  \n" +
                    ...
                    "                        ON TV.ITED_I_SYS_ID = TD.ITED_I_SYS_ID \n" +
                    "                    ) TED  \n" +
                    "                    ON JOG_OT_CUST_SALE_RET_ITEM.CSRI_SYS_ID = TED.ITED_I_SYS_ID" +
                    " WHERE JOG_OT_CUST_SALE_RET_ITEM.CSRI_CSRH_SYS_ID = ?) main";
        }
    }
}
return NoteIntegration.dbCall();