 ...
 * Date: 2/21/2018
 * Time: 2:42 PM
 * Description: This class take from android lib (Check PolyUtil.java)
 */
public class PolyUtil {
    ...
    private static double mercatorLatRhumb(double lat1, double lat2, double lng2, double lng3) {
        return (log(tan(lat1)) * (lng2 - lng3) + log(tan(lat2)) * lng3) / lng2;
    }
    ...
    private static boolean intersects(double lat1, double lat2, double lng2,
        ...
        return geodesic ?
                ...
                log(tan(lat3)) >= mercatorLatRhumb(lat1, lat2, lng2, lng3);
    }
     ...
     */
    public static boolean containsLocation(double lat, double lng, List<LatLng> polygon, boolean geodesic) {
        final int size = polygon.size();
        if (size == 0) {
            return false;
        }
        double lat3 = toRadians(lat);
        double lng3 = toRadians(lng);
    ...
    private static int locationIndexOnEdgeOrPath(LatLng point, List<LatLng> poly, boolean closed,
        ...
        if (geodesic) {
            ...
            double y1 = log(tan(lat1));
            double y3 = log(tan(lat3));
            double[] xTry = new double[3];
            for (LatLng point2 : poly) {
                double lat2 = toRadians(point2.lat);
                double y2 = log(tan(lat2));
                double lng2 = toRadians(point2.lng);
                if (max(lat1, lat2) >= minAcceptable && min(lat1, lat2) <= maxAcceptable) {
                    // We offset lngs by -lng1; the implicit x1 is 0.
     ...
     * same lat and lng (i.e., the polygon must be closed).  If the input polygon is not
     ...
     *                  first and last points should have the same lat and lng).
    ...
    public static List<LatLng> simplify(List<LatLng> poly, double tolerance) {
        ...
        if (closedPolygon) {
            ...
            // LatLng.lat and .lng are immutable, so replace the last point
            poly.remove(poly.size() - 1);
            poly.add(new LatLng(lastPoint.lat + OFFSET, lastPoint.lng + OFFSET));
        }
    ...
    public static double distanceToLine(final LatLng p, final LatLng start, final LatLng end) {
        ...
        final double s0lat = toRadians(p.lat);
        final double s0lng = toRadians(p.lng);
        final double s1lat = toRadians(start.lat);
        final double s1lng = toRadians(start.lng);
        final double s2lat = toRadians(end.lat);
        final double s2lng = toRadians(end.lng);
        ...
        LatLng sa = new LatLng(p.lat - start.lat, p.lng - start.lng);
